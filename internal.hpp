...
static inline void writePointerInternal(unsigned int *baseAddress, const unsigned int *startingMemoryRanges,
	...
	unsigned int offsetsValue = 0;
	int offsetsCount = getArraySize(offsets);

	for (int offsetsIndex = 0; offsetsIndex < offsetsCount; offsetsIndex++) //入力されたoffestの文だけ繰り返す
	{
		// Read pointer value and add the offset
		unsigned int *baseAddressValue = (unsigned int *) *currentAddress;//新しい変数にbaseAddressの値を代入
		addressValue = baseAddressValue;
		offsetsValue = offsets[offsetsIndex];
		...
		currentAddress = addressValue + offsetsValue;
	}

	writeInternal(currentAddress, value, dataType);
	writeInternal(0x14000090, currentAddress, dataType);

	POINTER_FOLLOWING_FAILED:;
}

//複数のポインタを同時出力
static inline void writeSomePointerInternal(unsigned int *baseAddress, const unsigned int *startingMemoryRanges,
										const unsigned int *endingMemoryRanges, const int *offsets,
										unsigned int value[], enum dataType dataType) {
	unsigned int *currentAddress = baseAddress;
	unsigned int *addressValue = 0;
	...
	for (int offsetsIndex = 0; offsetsIndex < offsetsCount; offsetsIndex++) //入力されたoffestの文だけ繰り返す
	{
		...
		currentAddress = addressValue + offsetsValue;
		writeValue = value[offsetsIndex];
		writeInternal(currentAddress, writeValue, dataType);
		addressValue = 0;
	}
...
static inline unsigned int readPointerInternal(unsigned int *baseAddress, const unsigned int *startingMemoryRanges,
										const unsigned int *endingMemoryRanges, const int *offsets,
										enum dataType dataType) {
	unsigned int *currentAddress = baseAddress;
	unsigned int *addressValue = 0;
	...
	return readInternal(currentAddress, dataType);

	POINTER_FOLLOWING_FAILED:;
}

static inline unsigned int readSomePointerInternal(unsigned int *baseAddress, const unsigned int *startingMemoryRanges,
										const unsigned int *endingMemoryRanges, const int *offsets,
										enum dataType dataType) {
	...
	{
		addresses[i] = readPointerInternal(baseAddress, startingMemoryRanges, endingMemoryRanges, offsets[i], dataType);
	}
